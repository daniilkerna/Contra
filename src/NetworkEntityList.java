...
public class NetworkEntityList implements Serializable
{
    ...
    public void addSnipers(ArrayList <EnemySniper>  enemySnipers){
        for (EnemySniper p : enemySnipers){
            entities.add(new NetworkEntity(NetworkEntity.NetworkEntityType.SNIPER, p.playerDesc, p.getSniperPosition(), null ) );
        }
    }

    public void addRunners(ArrayList <EnemyRunner>  enemyRunners){
        for (EnemyRunner p : enemyRunners){
            entities.add(new NetworkEntity(NetworkEntity.NetworkEntityType.RUNNER, p.playerDesc, p.getPlayerPosition(), null ));
        }
    }

    public void addTurrets(ArrayList <EnemyTurret>  enemyTurrets){
        for (EnemyTurret p : enemyTurrets){
            entities.add(new NetworkEntity(NetworkEntity.NetworkEntityType.TURRET, null, p.getTurretWorldPos() , p.getTurretState()));
        }
    }

    public void addBullets(ArrayList <Bullet>  bullets){
        for (Bullet b : bullets){
            entities.add(new NetworkEntity(NetworkEntity.NetworkEntityType.BULLET, null, b.getBulletWorldPos() , null ));
        }
    }

    public void addPlayer(Player p1){
        entities.add(new NetworkEntity(NetworkEntity.NetworkEntityType.PLAYER, p1.playerDesc, p1.getPosition() , null ));
    }
