...
public class EnemySniper extends Player {
    ...
    public EnemySniper (World world , Player p1 , final int x , final int y){
        ...
        playerDesc = new Player.Descriptor(State.IDLE, Movement.NONE, HorizontalFacingDirection.LEFT, VerticalFacingDirection.NONE);
    ...
    public void getState(GameContainer gc, StateBasedGame sbg, int delta ){
        if (refPlayer.getX() < getX()){
            this.playerDesc.hfd = HorizontalFacingDirection.LEFT;
            this.playerDesc.movement = Movement.LEFT;
        }
        else{
            this.playerDesc.hfd = HorizontalFacingDirection.RIGHT;
            this.playerDesc.movement = Movement.RIGHT;
        }

        if (refPlayer.getY() < getCoarseGrainedMinY()){
            this.playerDesc.vfd = VerticalFacingDirection.UP;
            this.playerDesc.state = State.RUNNING;
        }
        else if (refPlayer.getY() > getCoarseGrainedMaxY()){
            this.playerDesc.vfd = VerticalFacingDirection.DOWN;
            this.playerDesc.state = State.RUNNING;
        }
        else{
            this.playerDesc.vfd = VerticalFacingDirection.NONE;
        }
    }
